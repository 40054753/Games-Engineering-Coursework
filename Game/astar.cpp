...
vector<Vector2i> pathFind(Vector2i start, Vector2i finish) {
	...
	//This may not be the most efficient mechanism, but should not be a problem
	//unless the maps get very big.
		...
		for (auto& dir : directions) {
			...
			//Generate new node
				...
				if (open_nodes_map[next.x][next.y] == 0) {
					...
					//Now replace the node via swapping across the list and ignoring the old one
